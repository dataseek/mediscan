    ...
    <main className="relative box-border mx-auto flex min-h-screen w-full max-w-[min(26.25rem,calc(100vw-1.25rem))] flex-col bg-ink pb-[max(1.25rem,env(safe-area-inset-bottom,0px))] pl-[max(0.75rem,env(safe-area-inset-left,0px))] pr-[max(0.75rem,env(safe-area-inset-right,0px))] pt-[max(0.75rem,env(safe-area-inset-top,0px))] text-white antialiased sm:max-w-[26.25rem] sm:px-5 sm:pb-8 sm:pt-8">
      <div className="min-w-0 space-y-5">
        ...
        <div className="space-y-2">
          <h1 className="text-[22px] font-semibold tracking-[-0.04em]">Ingresar</h1>
          <p className="text-[13px] leading-relaxed text-[#9aa3b2]">
            Creá tu cuenta gratis para guardar tu historial y seguir analizando estudios.
          </p>
        </div>
        ...
        <div className="relative">
          ...
          <div className="relative flex justify-center">
            <span className="bg-ink px-3 text-[11px] font-medium text-white/50">o por email</span>
          </div>
        </div>
        ...
        <form
          ...
          onSubmit={async (event) => {
            ...
            try {
              ...
              setError(caught instanceof Error ? caught.message : "No pudimos enviar el link. Probá nuevamente.");
            } finally {
              setIsSending(false);
            }
          }}
        >
          ...
          >
            {isSending ? "Enviando link..." : "Enviar link mágico"}
          </button>
        </form>
      </div>
    </main>
